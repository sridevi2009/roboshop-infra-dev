...
resource "aws_cloudfront_distribution" "gopisri" {
  enabled             = true
  ...
  records = [
    {
      ...
      type = "A"
      alias = {
        name = aws_cloudfront_distribution.gopisri.domain_name
        zone_id = aws_cloudfront_distribution.gopisri.hosted_zone_id
      }
    }
  ]
}